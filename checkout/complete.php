...
if (isset($_GET["mailer_test"])) {
   try {
        $mailer = new OrderConfirmationMailer(24);
        $mailer->send();
    }
    catch (Exception $em) {
        echo ("Unable to email order confirmation message to user: ".$em->getMessage());
    } 
    exit;
}
...
echo "<BR><BR>";
echo tr("Можете да разгледате поръчките си от секция - ")."<a href='/account/'>".tr("Клиенти")."</a>";
echo "<BR><BR>";
